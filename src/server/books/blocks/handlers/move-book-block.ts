...
export async function moveBookBlock(bookId: string, dto: MoveBookBlockDto) {
  ...
  if (blockIndex === -1)
    throw new NotFoundError(`No block exists with id "${dto.blockId}" in the source container.`, {
      blockId: dto.blockId,
    });
  const movedBlock = fromBlocks[blockIndex];

  const sameContainer = JSON.stringify(fromRef) === JSON.stringify(toRef);
  const remainingFromBlocks = fromBlocks
    .filter((block) => block.id !== dto.blockId)
  ...
  const nextToBlocks = [
    ...destinationBlocks.slice(0, insertAt),
    { ...movedBlock, order: insertAt },
    ...destinationBlocks.slice(insertAt),
  ].map((block, order) => ({ ...block, order }));
  ...
  try {
    return await bookRepository.update(
      ...
      { ...patch, contentRevision: dto.expectedRevision + 1 },
    );
  } catch (error) {
