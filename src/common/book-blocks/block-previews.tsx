...
export function TextBlockPreview({ block }: { block: BookBlock }) {
  ...
  return (
    <Tag className="text-right font-semibold" dir="rtl">
      {block.text}
    </Tag>
  );
}
  ...
  if (block.type === BookBlockType.PARAGRAPH) {
    return (
      <div
        ...
        dir="rtl"
        dangerouslySetInnerHTML={{ __html: renderRichTextToHtml(block.richText) }}
      />
    );
  }
  if (block.type === BookBlockType.QUOTE) {
    return (
      <blockquote className="border-r-2 border-slate-300 pr-3 text-right text-sm italic" dir="rtl">
        ...
        {block.attribution ? (
          <cite className="mt-1 block text-xs not-italic text-slate-500">— {block.attribution}</cite>
        ) : null}
      </blockquote>
    );
  }
...
export function ImageBlockPreview({ block }: { block: BookBlock }) {
  ...
  return (
    <figure className="text-right" dir="rtl">
      {block.image ? (
        <img src={block.image.secureUrl} alt={block.caption ?? ""} className="max-h-40 rounded-md" />
      ) : (
...
export function SimpleBlockPreview({ block }: { block: BookBlock }) {
  if (block.type === BookBlockType.PAGE_BREAK) {
    return (
      <div className="border-t border-dashed border-slate-400 py-2 text-center text-xs text-slate-500">
        ──── بداية صفحة جديدة ────
      </div>
    );
  }
...
export function RecipeRefBlockPreview({ block }: { block: BookBlock }) {
  ...
  return (
    <p className="text-right text-sm" dir="rtl">
      {block.displayTitle ?? `Recipe: ${block.recipeId}`}
    </p>
  );
}
...
export function CitationBlockPreview({ block }: { block: BookBlock }) {
  ...
  return (
    <p className="text-right text-xs text-slate-500" dir="rtl">
      Reference: {block.referenceId}
    </p>
  );
}

export function PageFooterNoteBlockPreview({ block }: { block: BookBlock }) {
  if (block.type !== BookBlockType.PAGE_FOOTER_NOTE) return null;
  return (
    <div className="rounded-md border border-dashed border-slate-300 p-2 text-right text-sm" dir="rtl">
      <p className="mb-1 text-xs font-medium text-slate-500">Footer note — لا يظهر هنا في مكانه الطبيعي</p>
      <div className="text-slate-700" dangerouslySetInnerHTML={{ __html: renderRichTextToHtml(block.richText) }} />
    </div>
  );
}
