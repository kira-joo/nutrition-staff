...
async function main() {
  ...
  console.log(
    `Current coverMode: ${book.coverMode}, coverImage: ${book.coverImage ? book.coverImage.secureUrl : "none"}`,
  );
  console.log(
    `Current backCoverMode: ${book.backCoverMode}, backCoverImage: ${book.backCoverImage ? book.backCoverImage.secureUrl : "none"}`,
  );

  const updated = await bookRepository.update(
    { where: { _id: BOOK_ID, revision: book.revision } },
    { coverMode: "uploaded", revision: book.revision + 1 },
  );
