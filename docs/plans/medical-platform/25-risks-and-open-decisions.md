...
| R11 | **Implicit organization scoping can hide a tenant leak** more effectively than explicit filtering would.                                                                                                                                                                                                                                                                                                                       | Medium                                                   | Two layers: automatic injection _and_ a required indexed field so an unscoped write fails at the schema. Cross-org isolation tests on every list endpoint, written while there is still one org. Codex specifically asked to challenge this. [06](06-organization-and-branches.md)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
sufficient proof of the _mechanism_; a real second specialty should wait for a
...
10. ~~**A third-party calendar library.**~~ **This rejection was itself rejected
    ...
    What _is_ rejected is **shipping a calendar in a shared package**: a Phase 6
...
28. **A global `User` with `OrganizationMembership` in v1.** One deployment means
    ...
    not solve the cross-_deployment_ identity problem it was proposed for. See D9.
...
| ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------- |
| **HTTP-level idempotency.** Transaction retries do not protect against a _retransmitted request_ — a double-clicked "Take payment" is two HTTP requests and two successful transactions. | `IdempotencyKey` entity ([04](04-domain-model.md)); applied to booking, check-in, sign, issue, payment, refund, registration; tested per route ([22](22-testing-and-qa.md))                                                                                                                                                                                                                 | **In v1**                                     |
...
- **Production patient data existing before Phase 5.** Would make M8 and M9
  ...
  all — the rename is cheap _only_ while the data is small.
...
_"An independent reviewer used as a rubber stamp is worth nothing."_ The review
